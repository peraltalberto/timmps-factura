...
public class CliPedidosLinJpaController implements Serializable {
    ...
    public CliPedidosLinJpaController(UserTransaction utx, EntityManagerFactory emf) {
        this.utx = utx;
        this.emf = emf;
    }
    ...
    private List<CliPedidosLin> findCliPedidosLinEntities(boolean all, int maxResults, int firstResult) {
        ...
        try {
            ...
            cq.select(cq.from(CliPedidosLin.class));
    ...
    public int getCliPedidosLinCount() {
        ...
        try {
            CriteriaQuery cq = em.getCriteriaBuilder().createQuery();
            Root<CliPedidosLin> rt = cq.from(CliPedidosLin.class);
