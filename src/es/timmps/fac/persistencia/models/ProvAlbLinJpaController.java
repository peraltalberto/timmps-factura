...
public class ProvAlbLinJpaController implements Serializable {
    ...
    public ProvAlbLinJpaController(EntityManagerFactory emf) {
        this.emf = emf;
    }
    ...
    private List<ProvAlbLin> findProvAlbLinEntities(boolean all, int maxResults, int firstResult) {
        ...
        try {
            Query q = em.createQuery("select object(o) from ProvAlbLin as o");
            if (!all) {
    ...
    public int getProvAlbLinCount() {
        EntityManager em = getEntityManager();
        try {
            Query q = em.createQuery("select count(o) from ProvAlbLin as o");
