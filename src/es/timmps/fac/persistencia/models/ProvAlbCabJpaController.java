...
public class ProvAlbCabJpaController implements Serializable {
    ...
    public ProvAlbCabJpaController(EntityManagerFactory emf) {
        this.emf = emf;
    }
    ...
    private List<ProvAlbCab> findProvAlbCabEntities(boolean all, int maxResults, int firstResult) {
        ...
        try {
            Query q = em.createQuery("select object(o) from ProvAlbCab as o");
            if (!all) {
    ...
    public int getProvAlbCabCount() {
        EntityManager em = getEntityManager();
        try {
            Query q = em.createQuery("select count(o) from ProvAlbCab as o");
